...
for (let i = 0; i < schemas.length; i += 1) {
  ...
  test("generateJson works with all arguments", () => {
    faker.seed(SEED);
    const data = generateJson({ schema, keyReducers: keyReducer, count: 3 });
    expect(data).toMatchSnapshot();
  });
  test("generateJson works without keyReducers", () => {
    faker.seed(SEED);
    const data = generateJson({ schema, count: 3 });
    expect(data).toMatchSnapshot();
  });

  test("generateJson works without keyReducers & without count", () => {
    faker.seed(SEED);
    const data = generateJson({ schema });
    expect(data).toMatchSnapshot();
  });
}
